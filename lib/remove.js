    ...
    switch (options.type) {
      ...
      case 'stateless':
        if (mode === 'replace') {
          ...
          if (path.node.right.type === 'ConditionalExpression' && path.node.right.test.type === 'BinaryExpression' && path.node.right.test.left.type === 'MemberExpression' && path.node.right.test.left.object.type === 'MemberExpression' && path.node.right.test.left.object.object.type === 'Identifier' && path.node.right.test.left.object.object.name === 'process' && path.node.right.test.left.object.property.type === 'Identifier' && path.node.right.test.left.object.property.name === 'env') break;

          path.replaceWith(types.assignmentExpression('=', path.node.left, types.conditionalExpression(types.BinaryExpression('===', types.memberExpression(types.memberExpression(types.identifier('process'), types.identifier('env')), types.identifier('NODE_ENV')), types.stringLiteral('production')), types.objectExpression([]), path.node.right)));
          break;
        }
